...
public class MobilePhone {
    ...
    public boolean updateContact(Contact oldContact, Contact newContact) {
        ...
        int indexValue = findContact(oldContact);
        String name=newContact.getName();
        if (indexValue >= 0) {
             return false;
                } else if(findContact(name)!=-1)
                {
                    return false;
                }
        else {
                    this.myContacts.set(indexValue, newContact);
                    return true;
                }
    ...
    public boolean removeContact(Contact contact) {
        int indexValue = findContact(contact);
        if (indexValue >= 0) {
            this.myContacts.remove(indexValue);
            return true;
        } else {
            return false;
        }
    ...
    private int findContact(Contact contact) {
        int value = this.myContacts.indexOf(contact);
        return value;
    }

    private int findContact(String name) {
        for (int i = 0; i < this.myContacts.size(); i++) {
            Contact contactValue = this.myContacts.get(i);
            String personName = contactValue.getName();
    ...
    public Contact queryContact(String name) {
        int value = findContact(name);
        if (value >= 0) {
            return this.myContacts.get(value);
        }
        return null;
    ...
    public void printContacts() {
        ...
        for (int i = 0; i < this.myContacts.size(); i++) {
            Contact contactPerson = this.myContacts.get(i);
            System.out.println((i + 1) + ". " + contactPerson.getName()
